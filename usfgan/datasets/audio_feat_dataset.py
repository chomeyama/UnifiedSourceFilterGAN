...
from usfgan.utils import read_hdf5, read_txt, check_filename
from usfgan.utils import validate_length, batch_f0, dilated_factor
...
class AudioFeatDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        f0 = np.copy(feat[:, self.shift:self.shift+1])
        feat[:, self.shift:] = self.feat_transform(feat[:, self.shift:])

        if self.return_filename:
            items = self.feat_files[idx], audio, f0, feat, df
        else:
            items = audio, f0, feat, df
...
class FeatDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.return_filename:
            items = self.feat_files[idx], f0, feat, df
        else:
            items = f0, feat, df
