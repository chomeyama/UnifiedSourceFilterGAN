...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Decode dumped features with trained Quasi-Periodic Parallel WaveGAN Generator "
                    "(See detail in usfgan/bin/decode.py).")
    ...
    with torch.no_grad(), tqdm(dataset, desc="[decode]") as pbar:
        for idx, (feat_path, f0, c, d) in enumerate(pbar, 1):
            # setup input
            x = ()
            if input_type == "noise":
            ...
            feat_path = feat_path.replace(args.indir, args.outdir)
            if f0_factor == 1.0:  # unchanged
                wav_filename = f"{feat_path}_src.wav"
            else:  # scaled f0
                wav_filename = f"{feat_path}_f{f0_factor:.2f}_src.wav"
            if not os.path.exists(os.path.dirname(wav_filename)):
                os.makedirs(os.path.dirname(wav_filename))
            sf.write(wav_filename, s, config.get("sampling_rate", 22050), "PCM_16")
